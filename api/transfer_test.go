...
import (
	...
	mockdb "github.com/rachhen/simple_bank/db/mock"
	db "github.com/rachhen/simple_bank/db/sqlc"
	"github.com/rachhen/simple_bank/token"
...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, sql.ErrNoRows)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Times(1).Return(account2, sql.ErrNoRows)
				store.EXPECT().TransferTx(gomock.Any(), gomock.Any()).Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusNotFound, recorder.Code)
